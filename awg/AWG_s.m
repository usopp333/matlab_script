...
%n_sio2=1.4482;  %index of sio2
dlambda=0.01;  %信道间隔  影响到阵列波导的长度差，使得水平阵列波导的尺寸变化，间隔约大，尺寸越小；
N_out=1;  %输出端口数目
...
fsr=1.3*N_out*dlambda;  %FSR的选取（取2.5*保证小的串扰）
%fsr=0.1;  %FSR的选取
m_r=lambda/fsr+1;  %修正的衍射级数
m=round(m_r*n_b_eff/ng_b);  %衍射级数
%m=round(m_r*n_b_eff/n_b_eff);  %衍射级数
L_f=d_in*d_in*n_s_eff/(m_r*dlambda);  %大罗兰圆的半径
...
% 每个阵列波导的光场的振幅分布
for i= 1:N_a;
    array_t_am(i) = array_t_max(i)/(sum(array_t_max));   
end
figure;
plot(1:N_a,array_t_am,'*');
title('阵列波导的光场振幅分布');
